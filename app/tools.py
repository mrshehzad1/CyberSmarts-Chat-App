...
# Define the tools
async def get_answer_from_knowledgebase_handler(query: str) -> dict:
    webhook_url = os.environ.get("MAKE_COM_WEBHOOK_URL")

    try:
        response = requests.post(webhook_url, json={"query": query, "thread_id": "empty"})
        
        # Clean the response text before parsing
        cleaned_response = response.text.strip()
        if not cleaned_response:
            return {"error": "Empty response from webhook"}
        ...
        logger.error(f"Error fetching answer from knowledgebase: {error}")
        return {"error": str(error)}

get_answer_from_knowledgebase_def = {
    "name": "get_answer_from_knowledgebase_assistant",
    "description": "Get the answer of user query from the knowledgebase assistant",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": "get answer from knowledge base assistant"
            }
        },
        "required": ["query"]
    }
...
get_answer_from_knowledgebase = (get_answer_from_knowledgebase_def, get_answer_from_knowledgebase_handler)
